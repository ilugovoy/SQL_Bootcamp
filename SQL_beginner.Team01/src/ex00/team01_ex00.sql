-- -- ВАРИАНТ №1 С ИСПОЛЬЗОВАНИЕМ CTE
...
-- Выборка и вычисление итогового общего объема в USD с сортировкой
   SELECT
			COALESCE(usr.name, 'not defined') AS name, -- имя пользователя, (при отсутствии выводится "not defined") 
			COALESCE(usr.lastname, 'not defined') AS lastname, -- фамилия (при отсутствии "not defined")
			bal.type AS type, -- тип баланса
			bal.total_balance AS volume, -- общий баланс пользователя
			...
			CAST(ROUND(bal.total_balance * COALESCE(cur.last_rate_to_usd, 1), 6) AS real) AS total_volume_in_usd -- общий баланс пользователя в USD с округлением до 6 знаков
